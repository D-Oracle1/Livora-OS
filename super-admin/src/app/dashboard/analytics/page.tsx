...
export default function SuperAdminAnalytics() {
  ...
  return (
    <div className="space-y-6 p-3 sm:p-6">
      ...
      {/* Rankings — 1 col mobile, 2 col sm, 3 col lg */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {rankingColumns.map(({ title, icon: Icon, data, key }) => (
          <div key={title} className="neuo-card p-4 sm:p-5">
      ...
      <div className="neuo-card overflow-hidden">
        ...
        <ul className="divide-y divide-gray-100 sm:hidden">
          {companies.length === 0 ? (
            <li className="p-8 text-center text-sm text-gray-400">No companies yet</li>
          ) : companies.map((c) => (
            <li key={c.id} className="p-4 space-y-2">
              ...
              <div className="flex flex-wrap gap-3 text-xs text-gray-500">
                <span className="flex items-center gap-1">
                  ...
                  {c.stats?.users || 0} users
                </span>
                <span className="flex items-center gap-1">
                  <Home className="w-3 h-3" style={{ color: accent }} />
                  {c.stats?.properties || 0} props
                </span>
                <span className="flex items-center gap-1">
                  <NairaSign className="w-3 h-3" style={{ color: accent }} />
                  {c.stats?.sales || 0} sales
                </span>
                <span className="flex items-center gap-1">
